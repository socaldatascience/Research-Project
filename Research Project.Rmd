...
indx1 <- sapply(severity0, is.factor)
severity0[indx1] <- lapply(severity0[indx1], function(x) as.numeric(as.character(x)))

sev0_sum <- colSums(severity0)
```
...
indx0 <- sapply(severity1, is.factor)
severity1[indx0] <- lapply(severity1[indx], function(x) as.numeric(as.character(x)))

sev1_sum <- colSums(severity1)
```
...
indx <- sapply(severity2, is.factor)
severity2[indx] <- lapply(severity2[indx], function(x) as.numeric(as.character(x)))

sev2_sum <- colSums(severity2)
```

```{r severity lvls df}
#created new df based on counts of morbidities across severity lvls
library(reshape2)
sev_lvls <- rbind(sev0_sum, sev1_sum, sev2_sum)

melted_sev_lvls <- melt(sev_lvls)

colnames(melted_sev_lvls) <- c('severity_lvl', 'morbidity','obs')
```

```{r visual}
ggplot(melted_sev_lvls, aes(x=morbidity, y = obs)) +
